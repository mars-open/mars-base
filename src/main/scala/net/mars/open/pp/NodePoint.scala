...
import net.mars.open.pp.utils.GeometryCalcUtils.{calcAzimuth, calcCircumRadius}
 ...
 */
case class NodePoint(uuid_node: String, geometry: Geometry, geometry1: Geometry, azimuth: Float, radius: Option[Int], chord_length: Option[Float], end: Boolean, src_id: Option[String]) {
  def point: Point = geometry.asInstanceOf[Point]
}
object NodePoint {
  def from(uuid_node: String, p1: Coordinate, p2: Coordinate, p3: Option[Coordinate], end: Boolean, src_id: Option[String])(implicit factory: GeometryFactory): NodePoint = {
    val azimuth = calcAzimuth(p1, p2).toFloat
    val circum = p3.map(p3 => (calcCircumRadius(p1, p2, p3).map(_.toFloat), p1.distance(p3).toFloat))
    NodePoint(uuid_node, factory.createPoint(p1), factory.createPoint(p2), azimuth, circum.flatMap(_._1).map(_.toInt), circum.map(_._2), end, src_id)
  }
}