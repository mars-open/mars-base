...
class Tlm3dPpTransformerTest extends AnyFunSuite {
  ...
  test("EdgePoints for line1 are created in fixed interval of 0.25 distance, respecting min 0.25 distance at the end") {
    val result = Tlm3dPpTransformer.createPointsAtFixedInterval(0.25, 25, "EPSG:2056")("line", line1)
    val expected = Seq(
      EdgePoint(point (0, 0, 100d, 0.0), 0, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 0.25, 100d, 0.25), 3, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 0.5, 100d, 0.5), 3, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 0.75, 100d, 0.75), 3, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 1, 100d, 1), 2, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 1.25, 100d, 1.25), 3, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 1.5, 100d, 1.5), 3, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 1.75, 100d, 1.75), 3, None, Some(0.0f), 1.5707964f,true),
      EdgePoint(point (0, 2.05, 100d, 2.05), 0, None, Some(0.0f), 1.5707964f,true),
    )
  ...
  test("EdgePoints for square1 are created in fixed interval, including height/radius/grade/azimuth interpolation and position") {
    ...
    val expected = Seq(
      EdgePoint(point (-0.5, -0.5, 100d, 0.0),   0, None,    Some(0.0f),          0.0f,true),
      EdgePoint(point (-0.25, -0.5, 100d, 0.25), 3, Some(1), Some(0.0f),          0.0f,true),
      EdgePoint(point (0, -0.5, 100d, 0.5),      3, Some(1), Some(0.0f),          0.0f,true),
      EdgePoint(point (0.25, -0.5, 100d, 0.75),  3, Some(1), Some(0.0f),          0.0f,true),
      EdgePoint(point (0.5, -0.5, 100d, 1.0),    2, Some(1), Some(0.0070710676f), 0.7853982f,true),
      EdgePoint(point (0.5, -0.25, 102.5d, 1.25),3, Some(1), Some(0.01f),         1.5707964f,true),
      EdgePoint(point (0.5, 0, 105d, 1.5),       3, Some(1), Some(0.01f),         1.5707964f,true),
      EdgePoint(point (0.5, 0.25, 107.5d, 1.75), 3, Some(1), Some(0.01f),         1.5707964f,true),
      EdgePoint(point (0.5, 0.5, 110d, 2.0),     2, Some(1), Some(0.0f),          2.3561945f,true),
      EdgePoint(point (0.25, 0.5, 107.5d, 2.25), 3, Some(1), Some(-0.01f),        3.1415927f,true),
      EdgePoint(point (0, 0.5, 105d, 2.5),       3, Some(1), Some(-0.01f),        3.1415927f,true),
      EdgePoint(point (-0.25, 0.5, 102.5d, 2.75),3, Some(1), Some(-0.01f),        3.1415927f,true),
      EdgePoint(point (-0.5, 0.5, 100d, 3),      2, Some(1), Some(-0.0070710676f),-2.3561945f,true),
      EdgePoint(point (-0.5, 0.25, 100d, 3.25),  3, Some(1), Some(0.0f),          -1.5707964f,true),
      EdgePoint(point (-0.5, 0, 100d, 3.5),      3, Some(1), Some(0.0f),          -1.5707964f,true),
      EdgePoint(point (-0.5, -0.25, 100d, 3.75), 3, Some(1), Some(0.0f),          -1.5707964f,true),
      EdgePoint(point (-0.5, -0.5, 100d, 4.0),   0, None,    Some(0.0f),          -1.5707964f,true),
    )
    assert(result == expected)
    result.map(_.geometry).zip(expected.map(_.geometry)).foreach {
      case (p1, p2) =>
